...
> 日期：2026-09-23 ｜ 状态：已实现（M1 MVP，偏差见 §9） ｜ 交互基准：`prototype/reqspec.html`（可交互原型，含一键演示）
...
## 9. M1 实现偏差记录（2026-09-23 最终评审裁定，M1.x 处理）

| # | 偏差 | 原因 | M1.x 方向 |
|---|---|---|---|
| 1 | 「AI 生成文档」材料类型不可达（classify 不产该类型），错误边界 §6-2 链路断裂 | 智能分类无判定途径 | classify 增加途径（文件名 heuristics 或入池时手动标记 AI 生成） |
| 2 | 「代码与口头意图相反 → 强制转问人」闸门未实现 | 数据模型不可实现：Assertion 无来源维度（只有 conf 置信度），系统无法判定冲突是否「代码 vs 口头」 | Assertion 增加 source 字段（代码/口头/文档）；conflict 裁决路由按来源组合强制转问人 |
| 3 | 问人清单缺「推测断言」来源；clarify-gen AI 任务未实现（提问文本由代码拼装） | M1 只接通矛盾/空白两来源 | 推测/待实证断言加入问人池；prompts/clarify-gen.md |
| 4 | 断言无节点归属——组装吃全局断言，第二节点会污染第一节点卡片（R# 重复） | M1 单节点假设 | Assertion.node 字段 + 提取时绑定目标节点 + Fact 视图按节点过滤 |
| 5 | impact 任务已实现但无 API 路由与 UI——成功标准 2（增量第二单）延后 | M1 收敛范围 | 增量定位路由 + 基线页 UI（App.vue 已留位） |
| 6 | 卡片「变更属性」为前端硬编码占位（本次修改·高风险） | 无数据来源 | 卡片字段数据化（人工选择或从 diff 推断） |
| 7 | 节点徽章无后端聚合数据源（stats 预留未填） | 无聚合端点 | GET /tree 或独立端点聚合待确认/断言数 |
| 8 | 「落码验证」为手动标记（文案已改「标记已确认」，不虚构验证结论） | 真实验证需代码比对链路 | verify 时对 ref 关联断言跑 tasks.verify |
